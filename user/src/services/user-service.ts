...
export async function deleteUser(userId: string) {
  ...
  if(user.role === Role.tourGuide || user.role === Role.advertiser){
    ...
    if(await hasBookings(filter)){
      ...
      switch(user.role){
        case Role.tourGuide:
          errorMessage = "You have upcoming itineraries that have bookings, Itenraries will be deactivated";
          break;
        case Role.advertiser:
          errorMessage = "You have upcoming activities that have bookings, Activities will be deactivated";
          break;
      }
