...
import SortButtonStyles from "../styles/SortButton.module.css";
...
const Dropdown: React.FC<DropdownProps> = ({ onSort }) => {
  ...
  return (
    <div className={SortButtonStyles["dropdown-container"]}>
      <button className={SortButtonStyles["dropdown-button"]} onClick={() => setOpen(!open)}>
        Sort By
        <IoIosArrowUp
          className={`${SortButtonStyles["arrow-icon"]} ${open ? SortButtonStyles["flipped"] : ""}`}
        />
      </button>
