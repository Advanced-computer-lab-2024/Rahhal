  ...
  return (
    <div className="rounded-md border w-full h-[27rem] flex justify-between flex-col p-1 border-gray-300">
      <div className="flex flex-col">
          ...
          <Carousel className="w-full h-full">
            <CarouselContent className="w-full h-full ml-0">
              {hotel.images.map((image, imageIndex) => (
                <CarouselItem key={imageIndex} className="h-full w-full pl-0">
                  <img
                    src={image}
        ...
        <span className="text-md font-semibold truncate">{hotel.name}</span>
          <div className="flex justify-start items-center gap-2">
            ...
            <span className="text-sm text-gray-600">
              {hotel.rating.reviewCount.toLocaleString()}
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col">
          ...
          <div>
            <span className="text-lg font-semibold">
              {currencyExchangeSpec("USD", parseInt(hotel.averagePrice), rates, currency)
                ?.toFixed(0)
                .toLocaleString() +
                " " +
                currency || "N/A"}
