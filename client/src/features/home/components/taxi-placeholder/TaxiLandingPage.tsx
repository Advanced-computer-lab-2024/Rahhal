  ...
  return (
    <div className="w-full px-0 sm:px-8 lg:px-16 py-4">
      ...
      <div className="relative rounded-xl overflow-hidden w-full h-64 sm:h-80 lg:h-96">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 flex flex-col justify-end items-end px-8 sm:px-16 pb-8 sm:pb-16">
      ...
      <div className="py-8">
        ...
        <div className="relative w-full flex justify-center gap-6">
          {providers.map((provider, index) => (
            <div
              key={index}
              className="flex-none w-36 h-16 bg-white rounded-md flex items-center justify-start"
            >
              <img src={provider.src} alt={provider.alt} className="object-contain w-full h-full" />
            </div>
          ))}
        </div>
      </div>
